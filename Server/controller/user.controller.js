    ...
    try {
        const { name, email, password } = request.body
            ...
            const verifyEmail = await sendEmail({
              sendTo: email,
              subject: "Email verification from ELUS",
...
export async function forgotPasswordController(request, response){
    try {
        ...
        const expireTime = 10 * 60 * 1000 // 1hr
        ...
        await sendEmail({
          sendTo: email,
          subject: "Forgot password from ELUS",
...
export async function refreshToken(request, response){
    try {
        const refreshToken = refresh.cookies.refreshToken || request?.headers?.
        authorization.split( " ")(10) /// ''Bearer token
